...
def preprocess_weight(weight, bits=4):
    """
    Arguments:
        weight: (k, n) if bits == 8, (k, n / 2) if bits == 4, in int8
    Returns:
    ...
    if bits == 4:
        k, n = weight.shape[0], weight.shape[1] * 2
        assert n % 4 == 0 and k % 64 == 0
        # weight = weight.reshape(k // 32, 32, n // 2)[:, [0, 1, 8, 9, 16, 17, 24, 25, 2, 3, 10, 11, 18, 19, 26, 27, 4, 5, 12, 13, 20, 21, 28, 29, 6, 7, 14, 15, 22, 23, 30, 31], :].reshape(k, n // 2)
        # weight_8 = torch.stack([(weight << 4) >> 4, weight >> 4], dim=-1).reshape(k, n)
        # weight_8 = weight_8.T
        # weight_8 = rearrange(weight_8, "(nn four) (kk kblock) -> nn (kk four kblock)", four=4, kblock=64).reshape(n, k)
        # weight_8 = rearrange(weight_8, "(kk kblock) (nn four) -> nn (kk four kblock)", four=4, kblock=64).reshape(n, k)
        # weight_8 = (weight_8 + 8).reshape(n, k // 8, 8)[:, :, [0, 2, 4, 6, 1, 3, 5, 7]].reshape(n, k)
        ...
        weight = weight.reshape(k // 32, 32, n // 2)[:, perm, :].reshape(k, n // 2)
        weight_8 = torch.stack([(weight << 4) >> 4, weight >> 4], dim=-1).reshape(k, n) + 8
        weight = weight_8[::2, :] | (weight_8[1::2, :] << 4)
        weight = rearrange(weight, "(kk kblock) (nn four) -> nn (kk four kblock)", four=4, kblock=32).reshape(n, k // 2)
    else:
        k, n = weight.shape
        assert n % 2 == 0 and k % 64 == 0
        # weight = weight.reshape(k // 16, 16, n)[:, [0, 1, 8, 9, 2, 3, 10, 11, 4, 5, 12, 13, 6, 7, 14, 15], :].reshape(k, n)
        # weight = weight.T
        # weight = rearrange(weight, "(nn two) (kk kblock) -> nn (kk two kblock)", two=2, kblock=64).reshape(n, k)
        # weight = (weight + 128).reshape(n, k // 4, 4)[:, :, [0, 2, 1, 3]].reshape(n, k)
        perm = torch.tensor([0, 1, 8, 9, 2, 3, 10, 11, 4, 5, 12, 13, 6, 7, 14, 15]).reshape(-1, 4)[:, [0, 2, 1, 3]].reshape(-1).tolist()
        weight = weight.reshape(k // 16, 16, n)[:, perm, :].reshape(k, n)
        weight = rearrange(weight + 128, "(kk kblock) (nn two) -> nn (kk two kblock)", two=2, kblock=64).reshape(n, k)
