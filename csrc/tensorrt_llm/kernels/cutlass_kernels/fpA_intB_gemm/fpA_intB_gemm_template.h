    ...
    const int group_size, char* workspace_ptr, const size_t workspace_bytes, cudaStream_t stream)
{
    ...
    if (biases == nullptr) {
        run_gemm<tkc::EpilogueOpDefault>(
            A, B, weight_scales, weight_zero_points, nullptr, C, m, n, k, group_size, workspace_ptr, workspace_bytes, stream);
    } else {
        run_gemm<tkc::EpilogueOpBias>(
            A, B, weight_scales, weight_zero_points, biases, C, m, n, k, group_size, workspace_ptr, workspace_bytes, stream);
    }
}
